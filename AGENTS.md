...
- Status `TestSession` otomatis menjadi `selesai` saat nilai akhir mutu sudah keluar (ada minimal satu penilaian, `TestSession::nilaiAkhirMutuSudahKeluar()`), bukan lagi berdasar jumlah panelis yang ditugaskan. Dipicu saat panelis menyimpan (`Panelis\AssessmentController::store` via `TestSession::selesaikanOtomatisJikaLengkap()`), dan direkonsiliasi di banyak view admin & penyelia (`Admin\TestSessionController` `index`/`results`, `Penyelia\TestSessionController::show`, `Penyelia\MonitoringController::index`, `Penyelia\DashboardController::index`).
- Panelis hanya bisa menilai sesi berstatus `dibuka`. Sesi `draft`/`selesai` tidak muncul di daftar tugas dashboard panelis (`Panelis\DashboardController::index` filter `whereHas('testSession', status=dibuka)`) dan `Panelis\AssessmentController` `create`/`store` memakai `abort_unless(status === 'dibuka')`.
