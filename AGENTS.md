...
- Status `TestSession` otomatis menjadi `selesai` saat semua panelis (sessionUsers role=`panelis`) sudah mengirim penilaian — dipicu saat panelis menyimpan (`Panelis\AssessmentController::store` via `TestSession::selesaikanOtomatisJikaLengkap()`) dan direkonsiliasi saat admin membuka `index`/`show`/`results`.
